 ...
 *
 * Mutations are applied to an in-memory copy: approving an action really does
 * move the row, which keeps the UI honest about what a write does.
 */
...
let auditEvents: AuditEvent[] = buildAuditEvents();
...
function record(event: Omit<AuditEvent, 'id' | 'at'>): void {
  auditEvents = [
    { ...event, id: `evt_local_${auditEvents.length + 1}`, at: new Date().toISOString() },
    ...auditEvents,
  ];
}
...
export const seedAdapter: DataSource = {
  metrics: {
    getDashboardMetrics: (windowDays: number): Promise<DashboardMetrics> =>
      settle(buildMetrics(windowDays)),
    getTrend: (windowDays) => settle(buildTrend(windowDays)),
    getFailureBreakdown: (windowDays) => settle(buildFailureBreakdown(windowDays)),
    getMethodBreakdown: (windowDays) => settle(buildMethodBreakdown(windowDays)),
    getAttemptEffectiveness: () => settle(buildAttemptEffectiveness()),
  },
  ...
  recovery: {
    ...
    approveRecommendedAction: (jobId) => {
      const job = updateJob(jobId, {
        status: 'scheduled',
        nextActionAt: new Date(
          Date.now() + (jobs.find((j) => j.id === jobId)?.recommendedAction.delayMinutes ?? 15) * 60_000,
        ).toISOString(),
      });
      record({
  ...
  insights: {
    listInsights: (): Promise<Insight[]> => settle(buildInsights()),
  },
  ...
  system: {
    getEngineStatus: (): Promise<EngineStatus> => settle(buildEngineStatus()),
